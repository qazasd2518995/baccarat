...
import { playControlledDragonTigerRound, type BettingUserInfo } from '../utils/gameResultControl.js';
...
async function handleTableDealingPhase(io: TypedServer, tableId: string): Promise<void> {
  ...
  if (state.currentShoe.length < 10) {
    ...
    io.to(roomName).emit('dt:shuffle', { shoeNumber: state.shoeNumber });

    console.log(`[DT Table ${tableId}] New shoe #${state.shoeNumber} — stats reset`);
  ...
  const roundResult = await playControlledDragonTigerRound(state.currentShoe, bettingUsers);
  state.cardsRemaining = state.currentShoe.length;
  ...
  if (round && round.result) {
    ...
    for (const [userId, bets] of state.currentBets.entries()) {
      ...
      const finalBalance = await prisma.$transaction(async (tx) => {
        ...
        await distributeRebateForBets(tx as any, userId, totalBet, 'dragonTiger', savedRound.id);
